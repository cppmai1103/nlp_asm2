...
class CodeBERTTrainer:
    def __init__(self, task_subset='A', max_length=512, model_name="microsoft/codebert-base", parquet_path=None): #unixcoder-base
    ...
    def load_and_prepare_data(self):
        ...
        try:
            ...
            if self.parquet_path: 
                train_data = pd.read_parquet(self.parquet_path)
            else: 
                train_data = dataset['train']
            val_data = dataset['validation']
            logger.info(f"Loaded {len(train_data)} training samples")
            logger.info(f"Loaded {len(val_data)} validating samples")
            ...
            logger.info(f"Dataset columns: {df_train.columns.tolist()}")
            logger.info(f"Sample data:\n{df_train.head()}")
            
            if 'code' not in df_train.columns or 'label' not in df_train.columns:
                raise ValueError("Dataset must contain 'code' and 'label' columns")
            
            df_train = df_train.dropna(subset=['code', 'label'])
            df_val = df_val.dropna(subset=['code', 'label'])
            
            df_train['label'] = df_train['label'].astype(int)
            self.num_labels = df_train['label'].nunique()
            
            logger.info(f"Number of unique labels: {self.num_labels}")
            logger.info(f"Label range: {df_train['label'].min()} to {df_train['label'].max()}")
